...
pcapng_read(wtap *wth, int *err, gchar **err_info, gint64 *data_offset)
{
	...
	*data_offset = wth->data_offset;
	...
	/* read next block */
	while (1) {
		bytes_read = pcapng_read_block(wth->fh, FALSE, wth->capture.pcapng, &wblock, err, err_info);
		if (bytes_read <= 0) {
    ...
    int *err, gchar **err_info)
{
	...
	/* read the block */
	bytes_read = pcapng_read_block(wth->random_fh, FALSE, wth->capture.pcapng, &wblock, err, err_info);
	if (bytes_read <= 0) {
