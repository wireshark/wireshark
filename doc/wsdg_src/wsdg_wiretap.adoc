...
For convenience, we also added an entry to `app/wireshark_flavor.c`'s `wireshark_file_type_extensions_base` table so that this format can be selected with the 'open' dialog.
