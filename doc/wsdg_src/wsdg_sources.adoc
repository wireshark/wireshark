...
WIRESHARK_LOG_FATAL, or --log-fatal::
This is the level for which log messages are fatal. This can either be "critical" or "warning" level.

WIRESHARK_LOG_FATAL_COUNT, or --log-fatal-count::
This is the amount of fatal log messages which may pass before the program is trapped.
...
If you use an AI tool, add a trailer such as
[source,sh]
----
Assisted-by: Claude
----
to your commit message(s) and mention the same in merge requests.
The trailer should be omitted if no AI tools were used.
...
To include the "Assisted-by" trailer automatically, set the commit template with:
--
[source,sh]
----
...
The commit-msg hook warns if the "Assisted-by" trailer is missing.
...
If you want to automatically check for the latest version, you can use one of the {wireshark-code-file-url}/ui/software_update.h[automatic update URLs] for our Windows or macOS packages, such as
