...
The capture engine still lacks the following feature(s):
...
If you are unsure which options to choose in this dialog box, leaving the default settings as they are should work well in many cases.
...
Optimize::
Allows disabling the optimization applied by default when compiling the capture
filter into bytecode. This affects both the actual capture and the bytecode
shown in <<ChCapCompiledFilterOutputDialog,the Compiled Filter Output dialog>>.
This should usually be left enabled, but might be useful if you suspect a bug
in the optimizer, or for instructive purposes.
...
Processes:::
Allows you to record which processes on your computer sent or received each packet, for the packets of TCP and UDP sockets.
They are shown under “Process information” in the frame details, and can be filtered on with the `frame.process` fields.
By default the ID and name of each process are recorded, which is what tools such as netstat show.
You can also have the executable path, command line, parent process and user of each process recorded.
Be careful with that if the capture file will be given to somebody else: a command line can contain a password, and a path the name of a user.
Which processes can be identified depends on your privileges, and the packets of very short-lived sockets can be missed; see the `--process-info` option in the dumpcap man page for the details.
This requires the pcapng format.

Create a new file automatically...::
Sets the conditions for switching to a new capture file.
...
establishing phase is saved in one file and the things you would like to see are
...
|foo.cap|x|-|Multiple files, continuous|foo_00001_20260714110102.cap, foo_00002_20260714110318.cap, ...
|foo.cap|x|x|Multiple files, ring buffer|foo_00001_20260714110102.cap, foo_00002_20260714110318.cap, ...
|===
