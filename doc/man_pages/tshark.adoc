...
If used in conjunction with --global-profile, the specified profile will be loaded from the global configuration directory instead of the user's personal configuration directory.
(typically on UN*X'/usr/share/wireshark/profiles', On Windows, the directory is a profiles subdirectory of the directory in which Wireshark was installed, e.g., C:\Program Files\Wireshark\profiles.
On macOS, when running from an app bundle, it is the Contents/Resources/share/wireshark/profiles subdirectory within the app bundle.)

This option is useful in shared environments or automated testing setups where consistent configurations are needed, and user-specific settings should be avoided.
--
...
*split=*<proto> Split the output into one row per message instance of
the protocol <proto> within a packet.  This is useful when a single
packet contains multiple protocol messages (e.g., several Diameter
messages bundled in one SCTP packet).  Fields that belong to a specific
message instance are output only on that instance's row, while fields
not under any instance (e.g., frame-level fields) are repeated on every
row.  Only applies to *-T fields* CSV output.  By default, no splitting
is performed.
--
...
Field 2:: selector in the base for display
...
NOTE: *TShark* does not save preferences, so use this for temporary changes
...
*-z* dis,streams::
Collect statistics for all DIS radio streams discovered by the `dis` tap.
The output includes start and end times, endpoints, radio and entity IDs,
signal and transmitter packet counts, estimated lost packets, and jitter
metrics derived from DIS transmitter timestamps.
...
// tshark does not support the -d option to dump the filter, so no need to
// mention instructive purposes.
...
// This is not supported for extcaps (yet)

--process-info[=basic|full]::
+
--
Record, for each packet, the processes on this host that sent or received
it, when the packet belongs to a TCP or UDP socket: the process ID goes in
an __epb_processid_threadid__ option of the packet's block, and a Process
Information Block describing the process is written once per output file.
Wireshark shows them as the __frame.process__ fields.

With *basic*, which is the default, a process is described by what
identifies it: its ID, name and start time, which is what tools such as
*netstat* show.  With *full* its executable path, command line, parent
process ID and user are recorded as well.  Those can be sensitive: a command
line can contain a password or a token, and a path the name of a user.  They
are stored in the capture file as they are, for everybody the file is given
to, so use *full* only for files that will be handled accordingly.  With
*basic* they are not even read.  The argument has to be attached with an
equals sign, as in *--process-info=full*.

The processes are
found in the operating system's socket tables, so a socket must still exist
when its packet is captured; very short-lived sockets can be missed.  Which
processes can be identified depends on privileges: on Windows all of them;
on Linux the user's own, or all of them if *dumpcap* runs as root or has
the CAP_SYS_PTRACE capability, which it then keeps, and nothing else,
after opening the interfaces; on macOS the user's own, or all of them if
*dumpcap* runs as root.  Not supported on other platforms.
Requires the pcapng file format.
--
...
reduces output size and improves performance. (Only works with *-T json* and
*-T jsonraw*)
...
// Add a "capture-options" group for the common capture options?
// Note a few options have different behavior in tshark, like -w,
// because they operate on both live capture and when reading from file.
