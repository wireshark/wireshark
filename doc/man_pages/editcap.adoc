...
[ *--discard-all-secrets* ]
[ *--discard-name-resolution* ]
[ *--discard-process-info* ]
...
__esp__  ESP SA Key, see esp_sa file in your profile folder and https://ietf-opsawg-wg.github.io/draft-ietf-opsawg-pcap/draft-ietf-opsawg-pcapng.html#name-decryption-secrets-block
...
+
--
Discard all Name Resolution Block (NRB) entries from the input file when
writing the output file. This removes all hostname-to-IP address mappings
that were embedded in the capture file at capture time.
--

--discard-process-info::
+
--
Discard the process information from the input file when writing the
output file: the blocks that describe the processes on the capturing
host (as recorded by *dumpcap --process-info*, or by Darwin), and the
options of the packets that refer to them.  Use it before sharing a
capture file: the blocks can hold the names, executable paths, command
lines and users of the processes.
--
...
+
--
Split SCTP packets containing multiple bundled DATA chunks into individual
packets, one per DATA chunk. Each split packet retains the original Ethernet,
IP, and SCTP common headers. A packet comment indicating the original frame
number is added to each split packet. Non-SCTP packets and SCTP packets
without DATA chunks are passed through unchanged.
--
