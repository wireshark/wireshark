...
"TCP@<host>:<port>" causes B<TShark> to attempt to connect to the
...
optional I<filter> parameter, and only packets that match that filter
(and any capture filter or read filter) will be used in the calculations.
...
=item B<-z> bacapp_instanceid,tree[,I<filter>]
...
=item B<-z> bacapp_ip,tree[,I<filter>]
...
=item B<-z> bacapp_objectid,tree[,I<filter>]
...
=item B<-z> collectd,tree[,I<filter>]
...
=item B<-z> diameter,srt[,I<filter>]
...
I<filter> specifies the stream to be displayed. There are three formats:
...
=item B<-z> gsm_a,I<category>[,I<filter>]

Count the number of messages of each type in GSM A-I/F I<category>, which
can be one of:
...
=item B<-z> gsm_map,operation[,I<filter>]
...
=item B<-z> h225_ras,rtd[,I<filter>]
+
--
...
Example: B<tshark -z h225_ras,rtd>
...
=item B<-z> hart_ip,tree,[,I<filter>]
...
=item B<-z> smpp_commands,tree[,I<filter>]
...
=item B<-z> ucp_messages,tree[,I<filter>]
