...
(ws-buglink:14446[])
cve-idlink:2018-7337[]
// Fixed in master: 91409213ad
// Fixed in master-2.4: 511a8b0b54
...
// cp /dev/null /tmp/buglist.txt ; for bugnumber in `git log --stat v2.4.5rc0..| grep ' Bug:' | cut -f2 -d: | sort -n -u ` ; do ./tools/gen-bugnote $bugnumber; pbpaste >> /tmp/buglist.txt; done

* Change placement of "double chevron" in Filter Toolbar to eliminate overlap. (ws-buglink:14121[])

* AutoScroll does not work. (ws-buglink:14257[])

* BOOTP/DHCP: malformed packet -> when user class option (77) is present. (ws-buglink:14312[])

* GET MAX LUN wLength decoded as big-endian - USB Mass Storage. (ws-buglink:14360[])

* Unable to create Filter Expression Button for a yellow filter. (ws-buglink:14369[])

* Buildbot crash output: fuzz-2018-01-28-15874.pcap. (ws-buglink:14371[])

* NetScaler RPC segmentation fault / stack overflow. (ws-buglink:14399[])

* [oss-fuzz] #6028 RPC_NETLOGON: Direct-leak in g_malloc (generate_hash_key). (ws-buglink:14407[])

* Newline "{backslash}n" in packet list field increase line height for all rows. (ws-buglink:14424[])

* ieee80211-radio.c preamble duration calculation not correct. (ws-buglink:14439[])

* DIS: Malformed packet in SISO-STD-002 transmitter. (ws-buglink:14441[])
