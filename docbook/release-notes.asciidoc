...
// cp /dev/null /tmp/buglist.txt ; for bugnumber in `git log --stat v2.6.11rc0..| grep ' Bug:' | awk '{print $2}' | sort -n -u ` ; do "$(git rev-parse --show-toplevel)/tools/gen-bugnote" $bugnumber; pbpaste >> /tmp/buglist.txt; done
