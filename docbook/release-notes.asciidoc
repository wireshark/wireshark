...
* The Windows installers now ship with Npcap 0.991. They previously shipped
  with Npcap 0.99-r9.
...
// cp /dev/null /tmp/buglist.txt ; for bugnumber in `git log --stat v3.0.1rc0..| grep ' Bug:' | awk '{print $2}' | sort -n -u ` ; do "$(git rev-parse --show-toplevel)/tools/gen-bugnote" $bugnumber; pbpaste >> /tmp/buglist.txt; done
