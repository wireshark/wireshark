...
/* the window */
static GtkWidget *listen_rtp_dlg_w;
...
/****************************************************************************/
static void
listen_rtp_on_destroy(GtkObject *object _U_, gpointer user_data _U_)
{
	...
	gtk_widget_destroy(listen_rtp_dlg_w);
	main_scrolled_window = NULL;
	listen_rtp_dlg_w = NULL;
}
...
listen_rtp_init(voip_calls_tapinfo_t *voip_calls_tap)
{
	...
	listen_rtp_dlg_create();
	
}

/****************************************************************************/
/*  this function will be called using a pseudo dissector, so based on the parameters passed will do:
 *      voip_calls_tap=rtp=pinfo=NULL -> Reset and close the window if open (called when retapping or close)
 *	    voip_calls_tap != NULL -> listen_rtp_dlg_create (this is called when the user click "listen" in the VoIP calls)
 *	    rtp and pinfo != NULL -> every time there is an RTP packet tap by the VoipCalls
 */
static void listen_rtp_main(voip_calls_tapinfo_t *voip_calls_tap,struct _rtp_info *rtp_info, packet_info *pinfo)
{
	/* Init the pa and create the Dialog */
	if (voip_calls_tap) {
		listen_rtp_init(voip_calls_tap);
		return;
	}

	/* This is a RTP packet to be added */
	if( rtp_info && pinfo) {
		add_rtp_packet(rtp_info, pinfo);
		return;
	}

	/* All three parameters are NULL, so it is a Reset */
	reset_listen_rtp();
}
...
/****************************************************************************/
void proto_register_listen_rtp(void) {

	proto_listen_rtp = proto_register_protocol("Listen RTP", "ListenRTP", "listenrtp");
	register_dissector("listenrtp", listen_rtp_main, proto_listen_rtp);
}
