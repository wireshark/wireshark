   ...
   if ( !soe.anSoeHeaderControlUnion.Error )
   {
      if ( !soe.anSoeHeaderControlUnion.InComplete )
      {
         ...
         case ECAT_SOE_OPCODE_RRS:
            g_snprintf ( szText, nMax, "SoE: RRS (%s, '%s') : %lu Bytes", tmp, elm, soe_length-ETHERCAT_SOE_HEADER_LEN);
            break;
         case ECAT_SOE_OPCODE_WRS:
            g_snprintf ( szText, nMax, "SoE: WRS (%s, '%s')", tmp, elm);
            break;
         case ECAT_SOE_OPCODE_WRQ:
            g_snprintf ( szText, nMax, "SoE: WRQ (%s, '%s') : %lu Bytes", tmp, elm, soe_length-ETHERCAT_SOE_HEADER_LEN);
            break;
         case ECAT_SOE_OPCODE_NFC:
            g_snprintf ( szText, nMax, "SoE: NFC (%s, '%s') : %lu Bytes", tmp, elm, soe_length-ETHERCAT_SOE_HEADER_LEN);
            break;
         case 6:
