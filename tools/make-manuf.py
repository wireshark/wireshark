...
def shorten(manuf):
    ...
    return u'{:<22} # {}'.format(manuf, mixed_manuf)
...
def main():
    ...
    wka_path = os.path.join(this_dir, '..', 'wka.tmpl')
    ...
    manuf_fd.write(u"# Don't change it directly, change manuf.tmpl and wka.tmpl instead.\n#\n")
    manuf_fd.write('\n'.join(header_l))
    ...
    manuf_fd.write(u'\n')
    try:
        wka_fd = io.open(wka_path, 'r', encoding='UTF-8')
    except:
        exit_msg("Couldn't open wka template file for reading ({}) ".format(wka_path))
    for wka_line in wka_fd:
        wka_line = wka_line.rstrip()
        manuf_fd.write(u'{}\n'.format(wka_line))
