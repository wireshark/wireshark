...
/*
 * Table for the SC-32 CRC, also referred to as "CRC-32/6 corrected"
 * Defined in IEC 61784-3-3 (Safety fieldbuses aka ProfiSafe) and
 * referred in 61375-2-3 (TRDP SDTv2)
 * Xor{in,out}: no
 * Reflect{in,out}: no
 * Polynomial: 0x1F4ACFB13
 */

static const uint32_t crc32_sc32_table[256] = {
...
uint32_t
crc32_sc32_seed(const uint8_t *buf, unsigned len, uint32_t seed)
{
	unsigned i;
	uint32_t crc32;
	...
	for (i = 0; i < len; i++)
		crc32 = (crc32 << 8) ^ crc32_sc32_table[((crc32 >> 24) ^ buf[i]) & 0xff];
