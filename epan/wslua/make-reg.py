...
def generate_declare_wslua_h(classes, functions, internal_functions):
    ...
    for lua_class in classes:
        output_lines += [f"\tWSLUA_CLASS_DECLARE({lua_class});\\"]

    output_lines += ["\n"]
    output_lines += ["#define WSLUA_DECLARE_FUNCTIONS() \\"]
    for lua_function in functions:
        output_lines += [f"\tWSLUA_FUNCTION wslua_{lua_function}(lua_State* L);\\"]

    for lua_internal_function in internal_functions:
        output_lines += [f"\tWSLUA_INTERNAL_FUNCTION wslua_{lua_internal_function}(lua_State* L);\\"]
...
def generate_register_wslua_c(classes, functions, internal_functions):
    ...
    for lua_class in classes:
        output_lines += [f"\twslua_reg_module(L, \"{lua_class}\", {lua_class}_register);"]
    ...
    for lua_function in functions:
        output_lines += [f"\tWSLUA_REGISTER_FUNCTION({lua_function});"]

    for lua_internal_function in internal_functions:
        output_lines += [f"\tWSLUA_REGISTER_FUNCTION({lua_internal_function});"]
