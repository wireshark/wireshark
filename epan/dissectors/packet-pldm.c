...
int dissect_platform(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *p_tree,
	...
	guint8 offset = 0;
	guint8 pldm_cmd = tvb_get_guint8(tvb, offset);
	proto_tree_add_item(p_tree, hf_pldm_platform_commands, tvb, offset, 1, ENC_LITTLE_ENDIAN);
	offset += 1;
	if (!request) {//completion code in response only
		proto_tree_add_item(p_tree, hf_pldm_platform_completion_code, tvb, offset, 1,
				ENC_LITTLE_ENDIAN);
		guint8 completion_code = tvb_get_guint8(tvb, offset);
		if (completion_code)
			return tvb_captured_length(tvb);
	...
	switch (pldm_cmd) {
		...
		case 0x0a: // Platform Event Message command
			if (request) {
				proto_tree_add_item(p_tree, hf_pldm_platform_format_version, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				offset += 1;
				...
				proto_tree_add_item(p_tree, hf_event_class, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				guint8 platform_event_message_class = tvb_get_guint8(tvb, offset);
				offset += 1;
					...
					case 0x0: // SensorEvent(0x00)
						...
						switch (sensor_event_class){
							case 0x0: // Sensor Operational State
								proto_tree_add_item(p_tree, hf_sensor_present_op_state, tvb, offset, 1,
										ENC_LITTLE_ENDIAN);
								offset += 1;
					...
					case 0x4: // PLDM PDR Repository Change Event
						if (request) {
							proto_tree_add_item(p_tree, hf_pdr_data_format, tvb, offset, 1,
									ENC_LITTLE_ENDIAN);
							guint8 pdr_data_format = tvb_get_guint8(tvb, offset);
							offset += 1;
							proto_tree_add_item(p_tree, hf_pdr_num_change_recs, tvb, offset, 1,
									ENC_LITTLE_ENDIAN);
							guint8 num_change_record = tvb_get_guint8(tvb, offset);
							if(num_change_record>0){//if pdr_data_format is refresh entire repo then num-change-record shall be 0
								offset +=1;
								for(gint i = 0; i < num_change_record; i++){
									proto_tree_add_item(p_tree, hf_pdr_repo_change_event_data_op, tvb, offset, 1,
											ENC_LITTLE_ENDIAN);
									offset +=1;
									proto_tree_add_item(p_tree, hf_pdr_repo_change_rec_num_change_entries, tvb, offset, 1,
											ENC_LITTLE_ENDIAN);
									guint8 num_change_entries = tvb_get_guint8(tvb, offset);
									offset +=1;
									for(gint j = 0; j < num_change_entries; j++){
										if(pdr_data_format == 1){//pdr type enumeration  -- testing
											proto_tree_add_item(p_tree, hf_pdr_repo_change_event_record_pdr_type, tvb, offset, 4,
													ENC_LITTLE_ENDIAN);
										}
										else if(pdr_data_format == 2){// pdr Record handle enumeration
											proto_tree_add_item(p_tree, hf_pdr_repo_change_event_record_pdr_record_handle, tvb, offset, 4,
													ENC_LITTLE_ENDIAN);
										}
										offset+=4;
									}
		...
		case 0x21: // GetStateSensorReadings(33)
			if (request) {
				...
				if(cnt == 0){
					proto_tree_add_item(p_tree, hf_sensor_rearm_none, tvb, offset, 1,
							ENC_LITTLE_ENDIAN);
				}
				offset +=1;
				proto_tree_add_item(p_tree, hf_pldm_sensor_reserved, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
			} else {
				proto_tree_add_item(p_tree, hf_sensor_composite_count, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				guint8 sensor_comp_count = tvb_get_guint8(tvb, offset);
				for(gint i=0; i<sensor_comp_count; i++){ //statefield
					offset += 1;
					proto_tree_add_item(p_tree, hf_sensor_present_op_state, tvb, offset, 1,
							ENC_LITTLE_ENDIAN);
					offset += 1;
					proto_tree_add_item(p_tree, hf_sensor_present_event_state, tvb, offset, 1,
							ENC_LITTLE_ENDIAN);
					offset += 1;
					proto_tree_add_item(p_tree, hf_sensor_prev_event_state, tvb, offset, 1,
							ENC_LITTLE_ENDIAN);
					offset += 1;
					proto_tree_add_item(p_tree, hf_sensor_event_state, tvb, offset, 1,
							ENC_LITTLE_ENDIAN);
				}
			}
			break;
		case 0x11://GetSensorReading(17)
			if(request){
				proto_tree_add_item(p_tree, hf_sensor_id, tvb, offset, 2,
						ENC_LITTLE_ENDIAN);
				offset += 2;
				proto_tree_add_item(p_tree, hf_event_rearm, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
			}
			else{
				proto_tree_add_item(p_tree, hf_sensor_data_size, tvb, offset, 1,
				...
				proto_tree_add_item(p_tree, hf_sensor_present_op_state, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				offset += 1;
				proto_tree_add_item(p_tree, hf_sensor_event_msg_enable, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				offset += 1;
				proto_tree_add_item(p_tree, hf_sensor_present_event_state, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
		...
		case 0x32: //GetNumericEffecterValue(50)
			if(request){
				...
				guint8 size = tvb_get_guint8(tvb, offset);
				offset += 1;
				proto_tree_add_item(p_tree, hf_effecter_op_state, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				offset += 1;
				switch(size){
					case 0:
						proto_tree_add_item(p_tree, hf_effecter_value_pnd_u8, tvb, offset, 1,
								ENC_LITTLE_ENDIAN);
						offset += 1;
						proto_tree_add_item(p_tree, hf_effecter_value_pres_u8, tvb, offset, 1,
								ENC_LITTLE_ENDIAN); //todo ==> both hf is printing "effecter val" although one is prev or something
						break;
					case 1:
						proto_tree_add_item(p_tree, hf_effecter_value_pnd_s8, tvb, offset, 1,
								ENC_LITTLE_ENDIAN);
						offset += 1;
						proto_tree_add_item(p_tree, hf_effecter_value_pres_s8, tvb, offset, 1,
								ENC_LITTLE_ENDIAN);
						break;
					case 2:
						proto_tree_add_item(p_tree, hf_effecter_value_pnd_u16, tvb, offset, 2,
								ENC_LITTLE_ENDIAN);
						offset += 2;
						proto_tree_add_item(p_tree, hf_effecter_value_pres_u16, tvb, offset, 2,
								ENC_LITTLE_ENDIAN);
						break;
					case 3:
						proto_tree_add_item(p_tree, hf_effecter_value_pnd_s16, tvb, offset, 2,
								ENC_LITTLE_ENDIAN);
						offset += 2;
						proto_tree_add_item(p_tree, hf_effecter_value_pres_s16, tvb, offset, 2,
								ENC_LITTLE_ENDIAN);
						break;
					case 4:
						proto_tree_add_item(p_tree, hf_effecter_value_pnd_u32, tvb, offset, 4,
								ENC_LITTLE_ENDIAN);
						offset += 4;
						proto_tree_add_item(p_tree, hf_effecter_value_pres_u32, tvb, offset, 4,
								ENC_LITTLE_ENDIAN);
						break;
					case 5:
						proto_tree_add_item(p_tree, hf_effecter_value_pnd_s32, tvb, offset, 4,
								ENC_LITTLE_ENDIAN);
						offset += 4;
						proto_tree_add_item(p_tree, hf_effecter_value_pres_s32, tvb, offset, 4,
								ENC_LITTLE_ENDIAN);
						break;
					default: //Invalid
						col_append_fstr(pinfo->cinfo, COL_INFO, "Invalid byte");
		...
		case 0x39: // SetStateEffecterStates(57)
			if (request) {
				...
				proto_tree_add_item(p_tree, hf_effecter_count, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				guint8 effecter_comp_count = tvb_get_guint8(tvb, offset);
				for(gint i=0; i < effecter_comp_count; i++){ //statefield
					offset += 1;
					proto_tree_add_item(p_tree, hf_effecter_set_request, tvb, offset, 1,
							ENC_LITTLE_ENDIAN);
					offset += 1;
					proto_tree_add_item(p_tree, hf_effecter_state, tvb, offset, 1,
							ENC_LITTLE_ENDIAN);
				}
			}
			break;
		case 0x51: // GetPDR
			if (request) {
				proto_tree_add_item(p_tree, hf_pdr_record_handle, tvb, offset, 4,
						ENC_LITTLE_ENDIAN);
				offset += 4;
				proto_tree_add_item(p_tree, hf_pdr_data_handle, tvb, offset, 4,
						ENC_LITTLE_ENDIAN);
				offset += 4;
				proto_tree_add_item(p_tree, hf_pdr_transfer_op_flag, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				offset += 1;
				proto_tree_add_item(p_tree, hf_pdr_req_count, tvb, offset, 2,
						ENC_LITTLE_ENDIAN);
				offset += 2;
				proto_tree_add_item(p_tree, hf_pdr_record_change_num, tvb, offset, 2,
						ENC_LITTLE_ENDIAN);

			} else {
				proto_tree_add_item(p_tree, hf_pdr_next_record_handle, tvb, offset, 4,
						ENC_LITTLE_ENDIAN);
				offset += 4;
				proto_tree_add_item(p_tree, hf_pdr_next_data_handle, tvb, offset, 4,
						ENC_LITTLE_ENDIAN);
				offset += 4;
				proto_tree_add_item(p_tree, hf_pdr_transfer_flag, tvb, offset, 1,
						ENC_LITTLE_ENDIAN);
				guint8 transfer_flag = tvb_get_guint8(tvb, offset);
				offset += 1;
				proto_tree_add_item(p_tree, hf_pdr_response_count, tvb, offset, 2,
						ENC_LITTLE_ENDIAN);
				guint16 response_cnt = tvb_get_guint16(tvb, offset, ENC_LITTLE_ENDIAN);
				offset += 2;
				...
				if (response_cnt) {
					...
					while(response_cnt > 0){
						proto_tree_add_item(p_tree, hf_pdr_record_data, tvb, offset, 1,
								ENC_LITTLE_ENDIAN );
						offset += 1;
						response_cnt -= 1;
					}
		...
		if (reported_length >= 1) {
			switch(pldm_type){
				case 0:
					...
					break;
				case 2:
					dissect_platform(next_tvb, pinfo, pldm_tree, (void *)&d);
					break;
			}
		}
	}
...
void proto_register_pldm(void) {
	static hf_register_info hf[] = {
		...
		{&hf_pldm_platform_completion_code,
			{"Completion Code", "pldm.completionCode", FT_UINT8, BASE_DEC,
				VALS(platform_completion_codes), 0x0, NULL, HFILL}},
		...
		{&hf_event_class,
			{"Event Class", "pldm.platform.event.class", FT_UINT8, BASE_DEC,
				VALS(platform_event_message_classes), 0x0, NULL, HFILL}},
		{&hf_sensor_id,
		...
		{&hf_sensor_event_class,
			{"Sensor event class", "pldm.platform.event.sensor_event_class",
				FT_UINT8, BASE_DEC, VALS(sensor_platform_event_message_classes), 0x0, NULL, HFILL}},
		...
		{&hf_sensor_present_op_state,
			{"Sensor present operational state",
				"pldm.platform.event.sensor.op_state", FT_UINT8, BASE_DEC, VALS(platform_sensor_operational_state), 0x0,
				NULL, HFILL}},
		{&hf_sensor_prev_op_state,
			{"Sensor previous operational state",
				"pldm.platform.event.sensor.prev_op_state", FT_UINT8, BASE_DEC, VALS(platform_sensor_operational_state),
				0x0, NULL, HFILL}},
		{&hf_sensor_offset,
		...
		{&hf_event_prev_state,
			{"Event previous state", "pldm.platform.event.prev_state", FT_UINT8,
				BASE_DEC, VALS(pldm_sensor_event_states), 0x0, NULL, HFILL}},
		{&hf_sensor_data_size,
			{"Sensor data size", "pldm.platform.sensor.data_size", FT_UINT8,
				BASE_DEC, VALS(sensor_data_size), 0x0, NULL, HFILL}},
		{&hf_sensor_value_u8,
			{"Sensor reading", "pldm.platform.event.sensor.data_u8", FT_UINT8, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_sensor_value_s8,
			{"Sensor reading", "pldm.platform.event.sensor.data_s8", FT_INT8, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_sensor_value_u16,
			{"Sensor reading", "pldm.platform.event.sensor.data_u16", FT_UINT16,
				BASE_DEC, NULL, 0x0, NULL, HFILL}},
		{&hf_sensor_value_s16,
			{"Sensor reading", "pldm.platform.event.sensor.data_s16", FT_INT16, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_sensor_value_u32,
			{"Sensor reading", "pldm.platform.event.sensor.data_u32", FT_UINT32,
				BASE_DEC, NULL, 0x0, NULL, HFILL}},
		{&hf_sensor_value_s32,
			{"Sensor reading", "pldm.platform.event.sensor.data_s32", FT_INT32, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pnd_u8,
			{"Pending Effecter Value in uint8", "pldm.platform.effecter.pnd_val_u8", FT_UINT8, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pnd_s8,
			{"Pending Effecter Value in sint8", "pldm.platform.effecter.pnd_val_s8", FT_INT8, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pnd_u16,
			{"Pending Effecter Value in uint16", "pldm.platform.effecter.pnd_val_u16", FT_UINT16, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pnd_s16,
			{"Pending Effecter Value in sint16", "pldm.platform.effecter.pnd_val_s16", FT_INT16, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pnd_u32,
			{"Pending Effecter Value in uint32", "pldm.platform.effecter.pnd_val_u32", FT_UINT32, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pnd_s32,
			{"Pending Effecter Value in sint32", "pldm.platform.effecter.pnd_val_s32", FT_INT32, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pres_u8,
			{"Present Effecter Value in uint8", "pldm.platform.effecter.pres_val_u8", FT_UINT8, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pres_s8,
			{"Present Effecter Value in sint8", "pldm.platform.effecter.pres_val_s8", FT_INT8, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pres_u16,
			{"Present Effecter Value in uint16", "pldm.platform.effecter.pres_val_u16", FT_UINT16, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pres_s16,
			{"Present Effecter Value in sint16", "pldm.platform.effecter.pres_val_s16", FT_INT16, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pres_u32,
			{"Present Effecter Value in uint32", "pldm.platform.effecter.pres_val_u32", FT_UINT32, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_pres_s32,
			{"Present Effecter Value in sint32", "pldm.platform.effecter.pres_val_s32", FT_INT32, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_pdr_data_format,
			{"PDR Repository change data format",
				"pldm.platform.event.pdr.data_format", FT_UINT8, BASE_DEC, VALS(pldm_pdr_repository_chg_event_data_format), 0x0,
				NULL, HFILL}},
		...
		{&hf_pdr_repo_change_event_data_op,
			{"PDR Repository change event record - data operation",
				"pldm.platform.event.pdr.record.data_op", FT_UINT8, BASE_DEC, VALS(pdr_repo_chg_event_data_operation), 0x0,
				NULL, HFILL}},
		{&hf_pdr_repo_change_rec_num_change_entries,
			{"PDR Repository change event record - number of change entries",
				"pldm.platform.event.pdr.record.num_of_changes", FT_UINT8, BASE_DEC, NULL, 0x0,
				NULL, HFILL}},
		{&hf_pdr_repo_change_event_record_pdr_type,
			{"PDR Repository change event record-PDR Type",
				"pldm.platform.event.pdr.record.pdr_type", FT_UINT32, BASE_DEC, VALS(platform_pdr_type), 0x0,
				NULL, HFILL}},
		{&hf_pdr_repo_change_event_record_pdr_record_handle,
			{"PDR Repository change event record-PDR Record Handle",
				"pldm.platform.event.pdr.record.pdr_rec_handle", FT_UINT32, BASE_DEC, NULL, 0x0,
				NULL, HFILL}},
		{&hf_heartbeat_format_ver,
			{"Heartbeat Format Version", "pldm.platform.event.heartbeat.format_version",
				FT_UINT8, BASE_DEC, NULL, 0x0, NULL, HFILL}},
		...
		{&hf_effecter_value_u8,
			{"Effecter Value", "pldm.platform.effecter.value_u8", FT_UINT8, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_s8,
			{"Effecter Value", "pldm.platform.effecter.value_s8", FT_INT8, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_u16,
			{"Effecter Value", "pldm.platform.effecter.value_u16", FT_UINT16, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_s16,
			{"Effecter Value", "pldm.platform.effecter.value_s16", FT_INT16, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_u32,
			{"Effecter Value", "pldm.platform.effecter.value_u32", FT_UINT32, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_value_s32,
			{"Effecter Value", "pldm.platform.effecter.value_s32", FT_INT32, BASE_DEC,
				NULL, 0x0, NULL, HFILL}},
		{&hf_effecter_op_state,
			{"Effecter Operational State", "pldm.platform.effecter_op_state", FT_UINT8, BASE_DEC,
				VALS(pldm_effecter_oper_state), 0x0, NULL, HFILL}},
		{&hf_effecter_set_request,
			{"Effecter Set Request", "pldm.platform.effecter_set_req", FT_UINT8, BASE_DEC,
				VALS(pldm_effecter_state_set_request), 0x0, NULL, HFILL}},
		{&hf_effecter_state,
		...
		{&hf_pdr_transfer_flag,
			{"PDR transfer flag", "pldm.platform.pdr.transfer_flag", FT_UINT8,
				BASE_DEC, VALS(pdr_transfer_flags), 0x0, NULL, HFILL}},
