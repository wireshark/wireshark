...
void dissect_epath( tvbuff_t *tvb, packet_info *pinfo, proto_item *epath_item, int offset, int path_length, gboolean generate, cip_simple_request_info_t* req_data)
{
   ...
   {
      if (tvb_length_remaining(tvb, offset + pathpos) <= 0)
      {
         ...
         case CI_PORT_SEGMENT:
            ...
            if( segment_type & CI_PORT_SEG_EX_LINK_ADDRESS )
            {
               ...
               else
               {
                  ...
                  proto_tree_add_item( port_tree, hf_cip_link_address_string, tvb, offset+pathpos+2, opt_link_size, ENC_LITTLE_ENDIAN );
               }
         ...
         case CI_LOGICAL_SEGMENT:
            ...
            switch( segment_type & CI_LOGICAL_SEG_TYPE_MASK )
            {
               case CI_LOGICAL_SEG_CLASS_ID:
                  dissect_cia(tvb, offset, &pathpos, segment_type & CI_LOGICAL_SEG_FORMAT_MASK, generate, pinfo,
               ...
               case CI_DATA_SEG_SYMBOL:
                  ...
                  if( seg_size != 0 )
                  {
                     if ( generate )
                     {
                        ...
                        proto_tree_add_item( ds_tree, hf_cip_symbol, tvb, offset + pathpos + 2, seg_size, ENC_LITTLE_ENDIAN );
                         ...
                         attribute_info_t* attr, int offset, int total_len)
{
   ...
   {
      expert_add_info_format(pinfo, item, PI_UNDECODED, PI_ERROR, "PROGRAMMING ERROR: Attribute table set up incorrectly");
      return total_len;
   }
   ...
   case cip_usint_array:
      proto_tree_add_item(tree, *(attr->phf), tvb, offset, total_len, ENC_LITTLE_ENDIAN);
      consumed = total_len;
      break;
   case cip_uint_array:
   ...
   case cip_short_string:
      ...
      proto_tree_add_item(tree, *(attr->phf), tvb, offset+1, temp_data, ENC_LITTLE_ENDIAN);
      consumed = 1+temp_data;
      break;
   case cip_string:
      temp_data = tvb_get_letohs( tvb, offset );
      proto_tree_add_item(tree, *(attr->phf), tvb, offset+2, temp_data, ENC_LITTLE_ENDIAN);
...
dissect_cip_generic_data( proto_tree *item_tree, tvbuff_t *tvb, int offset, int item_length, packet_info *pinfo, proto_item *ti )
{
   ...
   else
   {
      ...
      if( (item_length-req_path_size-2) != 0 )
      {
         ...
         proto_tree_add_item(cmd_data_tree, hf_cip_data, tvb, offset+2+req_path_size, item_length-req_path_size-2, ENC_LITTLE_ENDIAN);
      }
      else
      {
         PROTO_ITEM_SET_HIDDEN( ti );
...
dissect_cip_class_generic(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
   ...
   if( tree )
   {
      ...
      ti = proto_tree_add_item(tree, proto_cip_class_generic, tvb, 0, -1, ENC_LITTLE_ENDIAN);
                                  ...
                                  int offset, cip_simple_request_info_t* req_data)
{
   ...
   if (attr != NULL)
   {
      dissect_cip_attribute(pinfo, tree, item, tvb, attr, offset, tvb_length_remaining(tvb, offset));
   }
   else
   {
      proto_tree_add_item(tree, hf_cip_sc_set_attr_single_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
   }
}
                                  ...
                                  int offset, cip_simple_request_info_t* req_data)
{
   ...
   for( i=0; i < att_count; i++ )
   {
      ...
      offset += 2;
      if ((tvb_length_remaining(tvb, offset+2) < 2) && (i < att_count-1))
      {
                                  ...
                                  int offset, cip_simple_request_info_t* req_data)
{
   ...
   for( i=0; i < att_count; i++ )
   {
      ...
      if (attr != NULL)
      {
         ...
         att_size = dissect_cip_attribute(pinfo, att_tree, att_item, tvb, attr, offset, tvb_length_remaining(tvb, offset));
         offset += att_size; 
      ...
      else
      {
         ...
         proto_tree_add_item(att_tree, hf_cip_sc_set_attr_list_attr_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      }

      if ((tvb_length_remaining(tvb, offset) < 2) && (i < att_count-1))
      {
                                  ...
                                  int offset, cip_simple_request_info_t* req_data _U_)
{
   ...
   for( i=0; i < num_services; i++ )
   {
      ...
      {
         /* Last service to add */
         serv_length = tvb_length_remaining(tvb, offset)-serv_offset;
      }
      else
      {
...
static int
dissect_cip_generic_service_req(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
   ...
   {
   case SC_GET_ATT_ALL:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_get_attribute_all_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_SET_ATT_ALL:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_set_attribute_all_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   ...
   case SC_RESET:
      ...
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_reset_data, tvb, offset+1, tvb_length_remaining(tvb, offset+1), ENC_LITTLE_ENDIAN);
      break;
   case SC_START:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_start_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_STOP:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_stop_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_CREATE:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_create_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_DELETE:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_delete_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_MULT_SERV_PACK:
      dissect_cip_multiple_service_packet_req(tvb, pinfo, cmd_data_tree, cmd_data_item, offset, &req_data);
      break;
   case SC_APPLY_ATTRIBUTES:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_apply_attributes_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_GET_ATT_SINGLE:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_get_attr_single_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   ...
   case SC_FIND_NEXT_OBJ_INST:
      ...
      break;
   case SC_RESTOR:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_restore_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_SAVE:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_save_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_NO_OP:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_noop_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_GET_MEMBER:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_get_member_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_SET_MEMBER:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_set_member_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_INSERT_MEMBER:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_insert_member_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_REMOVE_MEMBER:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_remove_member_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   case SC_GROUP_SYNC:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_group_sync_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   }
   ...
   for( i=0; i < att_count; i++ )
   {
      ...
      if (att_status == 0)
      {
         ...
         else
         {
            ...
            proto_tree_add_item(att_tree, hf_cip_sc_get_attr_list_attr_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
            break;
         }
      }

      if ((tvb_length_remaining(tvb, offset) < 4) && (i < att_count-1))
      {
         expert_add_info_format(pinfo, att_list, PI_MALFORMED, PI_ERROR, "Get Attribute List attribute list count greater than packet size");
         break;
      }
                                  ...
                                  int offset, cip_simple_request_info_t* req_data)
{
   ...
   {
      /* Can't find the attribute, treat the rest of the response as raw data */
      proto_tree_add_item(tree, hf_cip_sc_get_attr_single_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
   }
}
                                  ...
                                  int offset, cip_simple_request_info_t* req_data _U_)
{
   ...
   if (tvb_length_remaining(tvb, offset+((num_services+1)*2)) <= 0)
   {
   ...
   for( i=0; i < num_services; i++ )
   {
      ...
      if (tvb_length_remaining(tvb, serv_offset) <= 0)
      {
...
dissect_cip_find_next_object_rsp(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, proto_item * item, int offset)
{
   ...
   for (i = 0; i < num_instances; i++)
   {
      ...
      if ((tvb_length_remaining(tvb, offset) < 2) && (i < num_instances-1))
      {
...
/*      PROTO_ITEM_SET_HIDDEN( ti ); */
   ...
   {
   case SC_GET_ATT_ALL:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_get_attribute_all_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_SET_ATT_ALL:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_set_attribute_all_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_GET_ATT_LIST:
   ...
   case SC_SET_ATT_LIST:
      ...
      break;
   case SC_RESET:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_reset_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_START:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_start_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_STOP:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_stop_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_CREATE:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_create_instance, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_create_data, tvb, offset+4+add_stat_size+2, tvb_length_remaining(tvb, offset+4+add_stat_size+2), ENC_LITTLE_ENDIAN);
      break;
   case SC_DELETE:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_delete_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_MULT_SERV_PACK:
      dissect_cip_multiple_service_packet_rsp(tvb, pinfo, cmd_data_tree, cmd_data_item, offset+4+add_stat_size, &req_data);
      break;
   case SC_APPLY_ATTRIBUTES:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_apply_attributes_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_GET_ATT_SINGLE:
      dissect_cip_get_attribute_single_rsp(tvb, pinfo, cmd_data_tree, cmd_data_item, offset+4+add_stat_size, &req_data);
      break;
   case SC_SET_ATT_SINGLE:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_set_attr_single_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_FIND_NEXT_OBJ_INST:
      dissect_cip_find_next_object_rsp(tvb, pinfo, cmd_data_tree, cmd_data_item, offset+4+add_stat_size);
      break;
   case SC_RESTOR:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_restore_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_SAVE:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_save_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_NO_OP:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_noop_data, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      break;
   case SC_GET_MEMBER:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_get_member_data, tvb, offset, tvb_length_remaining(tvb, offset), ENC_LITTLE_ENDIAN);
      break;
   ...
   case SC_REMOVE_MEMBER:
      ...
      break;
   case SC_GROUP_SYNC:
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_group_sync_is_sync, tvb, offset+4+add_stat_size, tvb_length_remaining(tvb, offset+4+add_stat_size), ENC_LITTLE_ENDIAN);
      proto_tree_add_item(cmd_data_tree, hf_cip_sc_group_sync_data, tvb, offset+4+add_stat_size+1, tvb_length_remaining(tvb, offset+4+add_stat_size+1), ENC_LITTLE_ENDIAN);
      break;
   }
         ...
         if( add_stat_size )
         {
            ...
            default:
               ...
               if (add_stat_size > 1)
               {
                  ...
                  for( i=0; i < add_stat_size-1; i ++ )
                     proto_tree_add_item(add_status_tree, hf_cip_cm_add_status, tvb, offset+4+(i*2), 2, ENC_LITTLE_ENDIAN );
               }
            }
         }
      }
      ...
      if( ( item_length-4-add_stat_size ) != 0 )
      {
         ...
         if( gen_status == CI_GRC_SUCCESS || gen_status == CI_GRC_SERVICE_ERROR )
         {
           ...
           case SC_CM_LARGE_FWD_OPEN:
           {
               ...
               if (app_rep_size > 0)
                  proto_tree_add_item(cmd_data_tree, hf_cip_cm_app_reply_data, tvb, offset+4+add_stat_size+26, app_rep_size, ENC_LITTLE_ENDIAN );
           ...
           case SC_CM_FWD_CLOSE:
           {
               ...
               if (app_rep_size > 0)
                  proto_tree_add_item(cmd_data_tree, hf_cip_cm_app_reply_data, tvb, offset+4+add_stat_size+10, app_rep_size, ENC_LITTLE_ENDIAN);
...
dissect_cip_cco_all_attribute_common( proto_tree *cmd_tree, tvbuff_t *tvb, int offset, int item_length, packet_info *pinfo)
{
   ...
   if (config_data_size > 0)
      proto_tree_add_item(cmd_tree, hf_cip_cco_proxy_config_data, tvb, offset+variable_data_size+2, config_data_size, ENC_LITTLE_ENDIAN);
   ...
   if (config_data_size > 0)
      proto_tree_add_item(cmd_tree, hf_cip_cco_target_config_data, tvb, offset+variable_data_size+2, config_data_size, ENC_LITTLE_ENDIAN);
   ...
   iomap_tree = proto_item_add_subtree(iomapi, ett_cco_iomap);
      ...
      /* Attribute data */
      if (iomap_size > 0)
         proto_tree_add_item(iomap_tree, hf_cip_cco_iomap_attribute, tvb, offset+variable_data_size+4, iomap_size, ENC_LITTLE_ENDIAN);
   ...
   {
      /* Safety parameters */
      proto_tree_add_item(cmd_tree, hf_cip_cco_safety, tvb, offset+variable_data_size, 55, ENC_LITTLE_ENDIAN);
         ...
         if( gen_status == CI_GRC_SUCCESS || gen_status == CI_GRC_SERVICE_ERROR )
         {
                ...
                (req_data.iInstance != (guint32)-1))
            {
               if (req_data.iInstance == 0)
               {
                  ...
                  proto_tree_add_item(cmd_data_tree, hf_cip_cco_format_number, tvb, offset+4+add_stat_size+8, 2, ENC_LITTLE_ENDIAN );
                  proto_tree_add_item(cmd_data_tree, hf_cip_cco_edit_signature, tvb, offset+4+add_stat_size+10, 4, ENC_LITTLE_ENDIAN );
               }
               else
               {
...
dissect_cip_data( proto_tree *item_tree, tvbuff_t *tvb, int offset, packet_info *pinfo, cip_req_info_t* preq_info )
{
   ...
   else
   {
      ...
      {
         dissect_epath( tvb, pinfo, pi, offset+2, req_path_size*2, FALSE, NULL);
      }

      /* Take a sneak peak through the IOI to extract the class ID */
      ioilen = tvb_get_guint8( tvb, offset + 1 );
      if ( preq_info )
      ...
      if (service_index >= 0)
      {
          ...
          if(!dissector_try_heuristic(heur_subdissector_service, tvb, pinfo, item_tree))
          {
             dissect_cip_generic_service_req(tvb, pinfo, cip_tree);
          }
      }
      else if ( dissector )
      {
