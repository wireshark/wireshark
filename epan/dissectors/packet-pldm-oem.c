...
int
dissect_oem(tvbuff_t *tvb, packet_info *pinfo _U_, proto_tree *p_tree, void* data)
{
    ...
    case 0x1: //Get File Table
        if (request){
            proto_tree_add_item(p_tree, hf_file_data_handle, tvb, offset, 4, ENC_LITTLE_ENDIAN);
            offset +=4;
            proto_tree_add_item(p_tree, hf_file_transfer_op_flag, tvb, offset, 1, ENC_LITTLE_ENDIAN);
            offset +=1;
            proto_tree_add_item(p_tree, hf_file_table_type, tvb, offset, 1, ENC_LITTLE_ENDIAN);
        } else {
            proto_tree_add_item(p_tree, hf_file_next_data_handle, tvb, offset, 4, ENC_LITTLE_ENDIAN);
            offset +=4;
            proto_tree_add_item(p_tree, hf_file_transfer_flag, tvb, offset, 1, ENC_LITTLE_ENDIAN);
        }
        break;
    case 0x4: //Read File
    case 0x5: //Write File
        if (request){
            proto_tree_add_item(p_tree, hf_file_name_handle, tvb, offset, 4, ENC_LITTLE_ENDIAN);
	    offset += 4;
            proto_tree_add_item(p_tree, hf_file_offset, tvb, offset, 4, ENC_LITTLE_ENDIAN);
            offset +=4;
            proto_tree_add_item(p_tree, hf_file_length, tvb, offset, 4, ENC_LITTLE_ENDIAN);
	} else {
            proto_tree_add_item(p_tree, hf_file_length, tvb, offset, 4, ENC_LITTLE_ENDIAN);
	}
	break;
    ...
    case 0x7: //Write File from memory
	    ...
	    offset += 4;
            ...
            proto_tree_add_item(p_tree, hf_file_length, tvb, offset, 4, ENC_LITTLE_ENDIAN);
	    offset += 4;
            proto_tree_add_item(p_tree, hf_file_address, tvb, offset, 8, ENC_LITTLE_ENDIAN);
	} else {
            proto_tree_add_item(p_tree, hf_file_length, tvb, offset, 4, ENC_LITTLE_ENDIAN);
	}
	break;
    ...
    case 0x9: //Write File by type into memory
        if (request){
            proto_tree_add_item(p_tree, hf_file_type, tvb, offset, 2, ENC_LITTLE_ENDIAN);
	    offset += 2;
            proto_tree_add_item(p_tree, hf_file_handle, tvb, offset, 4, ENC_LITTLE_ENDIAN);
	    offset += 4;
            proto_tree_add_item(p_tree, hf_file_offset, tvb, offset, 4, ENC_LITTLE_ENDIAN);
            offset +=4;
            proto_tree_add_item(p_tree, hf_file_length, tvb, offset, 4, ENC_LITTLE_ENDIAN);
    ...
    case 0xa: //New File Available
	    ...
	    offset += 2;
            proto_tree_add_item(p_tree, hf_file_handle, tvb, offset, 4, ENC_LITTLE_ENDIAN);
	    offset += 4;
            proto_tree_add_item(p_tree, hf_file_length_64, tvb, offset, 8, ENC_LITTLE_ENDIAN);
	} else {
	}
	break;
...
proto_register_oem(void)
{
    static hf_register_info hf[] ={
        ...
        { &hf_file_data_handle,{
            "Data transfer handle", "pldm.file.table.handle",
            FT_UINT32, BASE_HEX,
            NULL, 0x0,
            NULL, HFILL}
         },
        { &hf_file_transfer_op_flag,{
            "Data transfer operation flag", "pldm.file.table.opflag",
            FT_UINT8, BASE_HEX,
            VALS(transfer_op_flags), 0x0,
            NULL, HFILL}
         },
        { &hf_file_table_type,{
            "File table type", "pldm.oem.table.type",
            FT_UINT8, BASE_HEX,
            VALS(file_table_types), 0x0,
            NULL, HFILL}
         },
        { &hf_file_next_data_handle,{
            "Next data transfer handle", "pldm.oem.table.nexthandle",
            FT_UINT32, BASE_HEX,
            NULL, 0x0,
            NULL, HFILL}
         },
        { &hf_file_transfer_flag,{
            "Data transfer operation flag", "pldm.file.table.flag",
            FT_UINT8, BASE_HEX,
            VALS(transfer_flags), 0x0,
            NULL, HFILL}
         },
        { &hf_file_type,{
            "File type", "pldm.file.type",
            FT_UINT16, BASE_HEX,
            NULL, 0x0,
            NULL, HFILL}
         },
        { &hf_file_name_handle,{
            "File name handle", "pldm.file.name_handle",
            FT_UINT32, BASE_HEX,
            NULL, 0x0,
            NULL, HFILL}
         },
        { &hf_file_handle,{
            "File handle", "pldm.file.handle",
            FT_UINT16, BASE_HEX,
            NULL, 0x0,
            NULL, HFILL}
         },
        { &hf_file_offset,{
            "File offset", "pldm.file.offset",
            FT_UINT32, BASE_HEX,
            NULL, 0x0,
            NULL, HFILL}
         },
        { &hf_file_length,{
            "File length", "pldm.file.length",
            FT_UINT32, BASE_HEX,
            NULL, 0x0,
            NULL, HFILL}
         },
        ...
        { &hf_file_address,{
            "File address", "pldm.file.address",
            FT_UINT64, BASE_HEX,
            NULL, 0x0,
            NULL, HFILL}
         },
    ...
    proto_pldm_oem = proto_register_protocol (
        "PLDM OEM Protocol", /* name        */
        "PLDM_OEM",          /* short_name  */
