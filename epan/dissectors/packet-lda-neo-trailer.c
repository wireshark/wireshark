...
dissect_lda_neo_trailer(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree, void *data _U_)
{
    ...
    /* Find the LDA Neo signature in the extra trailer bytes */
    ...
    if (offset > extra_trailer_bytes) {
        /* No valid LDA Neo signature found */
        return 0;
    }

    /* Construct LDA Neo tree */
    ...
    /* Construct LDA Neo sequence number */
    ...
    /* Construct LDA Neo PCS code tree and fields */
    ...
    /* Construct LDA Neo port tree and fields */
...
proto_register_lda_neo_trailer(void)
{
    ...
    proto_lda_neo_trailer = proto_register_protocol("LDA Neo Device trailer", "LDA Neo", "lda_neo_trailer");
