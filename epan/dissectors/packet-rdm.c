...
static unsigned
dissect_rdm_mdb_param_data_no_packed(tvbuff_t *tvb, unsigned offset, proto_tree *tree, uint8_t cc, uint16_t param_id, uint8_t pdl, uint16_t device_manufacturer_id);
  ...
  if (param_id < 0x8000) {
    ...
    *offset += 2;
    ...
    case RDM_MANUFACTURER_ID_ETC:
      proto_tree_add_item(tree, hf_etc_pd_parameter_id, tvb, *offset, 2, ENC_BIG_ENDIAN);
      *offset += 2;
      break;
    default:
      proto_tree_add_item(tree, hf_rdm_pd_parameter_id, tvb, *offset, 2, ENC_BIG_ENDIAN);
...
dissect_rdm_pd_device_info(tvbuff_t *tvb, unsigned offset, proto_tree *tree, uint8_t cc, uint8_t len _U_, uint16_t device_manufacturer_id)
{
  ...
  case RDM_CC_GET_COMMAND_RESPONSE:
    ...
    switch(device_manufacturer_id) {
    case RDM_MANUFACTURER_ID_ETC:
      proto_tree_add_item(tree, hf_etc_pd_device_model_id, tvb, offset, 2, ENC_BIG_ENDIAN);
      offset += 2;
      break;
    default:
...
static unsigned
dissect_rdm_pd_packed_pid_sub(tvbuff_t *tvb, unsigned offset, proto_tree *tree, uint8_t cc, uint8_t len, uint16_t device_manufacturer_id)
{
  uint16_t param_id = 0;
  ...
  if (cc == RDM_CC_GET_COMMAND_RESPONSE || cc == RDM_CC_SET_COMMAND) {
    while (len >= 3) {
      ...
      if (param_id == 0 || param_id == RDM_PARAM_ID_PACKED_PID_SUB || param_id == RDM_PARAM_ID_PACKED_PID_INDEX) {
        ...
        dissect_rdm_mdb_param_data_no_packed(tvb, offset, ti_tree, cc, param_id, param_len, device_manufacturer_id);
        offset += param_len;
      }
      len -= param_len;
    }
...
static unsigned
dissect_rdm_pd_packed_pid_index(tvbuff_t *tvb, unsigned offset, proto_tree *tree, uint8_t cc, uint8_t len, uint16_t device_manufacturer_id)
{
  uint16_t param_id = 0;
...
dissect_rdm_mdb_param_data(tvbuff_t *tvb, unsigned offset, proto_tree *tree, uint8_t cc, uint16_t param_id, uint8_t pdl, uint16_t device_manufacturer_id)
{
  ...
  case RDM_PARAM_ID_PACKED_PID_SUB:
    offset = dissect_rdm_pd_packed_pid_sub(tvb, offset, tree, cc, pdl, device_manufacturer_id);
    break;
  case RDM_PARAM_ID_PACKED_PID_INDEX:
    offset = dissect_rdm_pd_packed_pid_index(tvb, offset, tree, cc, pdl, device_manufacturer_id);
    break;
  default:
    offset = dissect_rdm_mdb_param_data_no_packed(tvb, offset, tree, cc, param_id, pdl, device_manufacturer_id);
    break;
  }
...
static unsigned
dissect_rdm_pd_ack_overflow(tvbuff_t *tvb, unsigned offset, proto_tree *tree, uint8_t cc, uint16_t param_id, uint8_t pdl, uint16_t device_manufacturer_id)
{
  ...
  */
  if (pdl > 0 && cc == RDM_CC_GET_COMMAND_RESPONSE) {
    ...
    case RDM_PARAM_ID_METADATA_JSON:
    case RDM_PARAM_ID_METADATA_JSON_URL:
      return dissect_rdm_mdb_param_data(tvb, offset, tree, cc, param_id, pdl, device_manufacturer_id);
    }
  }
  ...
  if (parameter_data_length > 0) {
    ...
    if (is_response(cc)) {
      ...
      case RDM_RESPONSE_TYPE_ACK:
        offset_data_end = dissect_rdm_mdb_param_data(tvb, offset, si, cc, param_id, parameter_data_length, device_manufacturer_id);
        break;
      ...
      case RDM_RESPONSE_TYPE_ACK_OVERFLOW:
        offset_data_end = dissect_rdm_pd_ack_overflow(tvb, offset, si, cc, param_id, parameter_data_length, device_manufacturer_id);
        break;
