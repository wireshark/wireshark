...
static void
dissect_rngrsp_commanded_power(tvbuff_t * tvb, packet_info* pinfo, proto_tree * tree, unsigned start, uint16_t len)
{
  uint16_t pos;
  uint16_t i;
...
dissect_rngrsp_tlv (tvbuff_t * tvb, packet_info * pinfo, proto_tree * rngrsp_tree)
{
  ...
  while (tvb_reported_length_remaining(tvb, pos) > 0)
  {
    ...
    case RNGRSP_EXT_US_COMMANDED_POWER:
      dissect_rngrsp_commanded_power(tvb, pinfo, rngrsptlv_tree, pos, tlvlen);
      break;
...
dissect_regrsp (tvbuff_t * tvb, packet_info * pinfo, proto_tree * tree, void* data _U_)
{
  ...
  col_add_fstr (pinfo->cinfo, COL_INFO,
                ...
                val_to_str_ext(pinfo->pool, response, &docsis_conf_code_ext, "%d"));
...
dissect_dsdrsp (tvbuff_t * tvb, packet_info * pinfo, proto_tree * tree, void* data _U_)
{
  ...
  col_add_fstr (pinfo->cinfo, COL_INFO,
                ...
                tranid, val_to_str_ext(pinfo->pool, confcode, &docsis_conf_code_ext, "%d"));
...
dissect_dbcrsp (tvbuff_t * tvb, packet_info * pinfo, proto_tree * tree, void* data _U_)
{
  ...
  col_add_fstr (pinfo->cinfo, COL_INFO,
                ...
                val_to_str_ext(pinfo->pool, confcode, &docsis_conf_code_ext, "%d"));
