...
dissect_macctrl(tvbuff_t *tvb, packet_info *pinfo, proto_tree *tree)
{
  ...
  proto_tree_add_uint(macctrl_tree, hf_macctrl_opcode, tvb, offset, 2, opcode);
  offset += 2;
  ...
  switch (opcode) {
    ...
    case MACCTRL_CLASS_BASED_FLOW_CNTRL_PAUSE:
      proto_tree_add_bitmask(macctrl_tree, tvb, offset, hf_macctrl_cbfc_enbv,
                             ett_macctrl_cbfc_enbv, macctrl_cbfc_enbv_list, ENC_BIG_ENDIAN);
      offset += 2;

      ti = proto_tree_add_text(macctrl_tree, tvb, offset, 8*2, "CBFC Class Pause Times");
